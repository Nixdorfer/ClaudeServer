...
type MCPServerInfo struct {
	...
	CustomOAuthClientID         interface{} `json:"custom_oauth_client_id"`
	HasCustomOAuthCredentials   bool        `json:"has_custom_oauth_credentials"`
	CustomOAuthClientSecretMask interface{} `json:"custom_oauth_client_secret_mask"`
}
...
func (c *MCPClient) ConnectToServer(serverUUID string) (*MCPConnection, error) {
	...
	if c.config.Debug {
		...
		if !hasCfClearance || !hasCfBm {
			log.Printf("⚠️  警告：Cookie 中缺少 Cloudflare 验证字段")
			if !hasCfClearance {
				log.Printf("   - 缺少 cf_clearance")
			}
			if !hasCfBm {
				log.Printf("   - 缺少 __cf_bm")
			}
			log.Printf("   MCP WebSocket 连接可能会失败")
		}
	}
	...
	if err != nil {
		if resp != nil {
			...
			if resp.StatusCode == 302 {
				log.Printf("💡 提示：收到 302 重定向，这通常是因为缺少 Cloudflare 验证 token")
				log.Printf("   请确保在配置文件的 tokens 部分添加了：")
				log.Printf("   - cf_clearance")
				log.Printf("   - cf_bm")
			} else if resp.StatusCode == 401 || resp.StatusCode == 403 {
				log.Printf("💡 提示：认证失败，请检查以下配置：")
				log.Printf("   - organization_id 是否正确")
				log.Printf("   - session_key 是否有效")
				log.Printf("   - Cookie 中的其他字段是否完整")
			}
			return nil, fmt.Errorf("websocket dial failed (status %d): %v", resp.StatusCode, err)
		}
		log.Printf("❌ WebSocket connection failed: %v", err)
		log.Printf("💡 提示：如果是网络错误，请检查：")
		log.Printf("   - 网络连接是否正常")
		log.Printf("   - 代理设置是否正确（如果使用代理）")
	...
	conn.SetCloseHandler(func(code int, text string) error {
		log.Printf("⚠️  WebSocket close handler triggered:")
	...
	for i := 0; i < 2; i++ {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "1006") {
				log.Printf("💡 提示：WebSocket 异常关闭 (1006)，可能的原因：")
				log.Printf("   1. Cloudflare 验证 token 缺失或过期（cf_clearance, cf_bm）")
				log.Printf("   2. 会话 token (session_key) 已过期")
				log.Printf("   3. Cookie 中缺少其他必需字段")
				log.Printf("   请尝试：")
				log.Printf("   - 重新从浏览器获取最新的 Cookie")
				log.Printf("   - 确保填写了所有显示在浏览器中的 Cookie 字段")
			}
...
func (conn *MCPConnection) ListTools() ([]interface{}, error) {
	conn.mu.Lock()
	requestID := conn.requestID
	conn.requestID++
	conn.mu.Unlock()
...
func (conn *MCPConnection) CallTool(toolName string, arguments map[string]interface{}) (map[string]interface{}, error) {
	conn.mu.Lock()
	requestID := conn.requestID
	conn.requestID++
	conn.mu.Unlock()
...
func (conn *MCPConnection) GetToolsForCompletion() ([]map[string]interface{}, error) {
...
func TestMCPConnection(config *Config, orgID, sessionKey, cookie, serverUUID string) error {
	...
	if config.Debug {
		...
		result, err := conn.CallTool("query_database", map[string]interface{}{
			"sql": "SELECT * FROM zm_merchant WHERE id=118;",
		})

		if err != nil {
			log.Printf("⚠️  Tool call test failed (this is optional): %v", err)
		} else {
