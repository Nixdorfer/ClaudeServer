...
function handleEscKey(e: KeyboardEvent) {
  if (e.key === 'Escape' && fullscreenIndex.value !== null) {
    closeFullscreen()
  }
}
onMounted(() => {
  ...
  >
    <div class="max-w-3xl mx-auto px-4 flex gap-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="message-content selectable-text text-zinc-200 break-words">
          <template v-for="(part, idx) in renderedContent.split(/<!--CODEBLOCK(\d+)-->/)" :key="idx">
            <template v-if="idx % 2 === 0">
              <span v-html="part"></span>
            </template>
            <template v-else>
              <div v-if="codeBlocks[parseInt(part)]" class="code-block-wrapper my-3">
                ...
                <template v-if="codeBlocks[parseInt(part)].isHtml && codeBlocks[parseInt(part)].showPreview">
                  <div class="html-preview bg-zinc-900 rounded-b-lg overflow-hidden relative">
                    <div class="flex items-center justify-between py-2 px-3 bg-zinc-800/50 border-b border-zinc-700">
                      <div class="flex items-center justify-center gap-2 flex-1">
                        <button
                          ...
                          :class="codeBlocks[parseInt(part)].previewMode === 'desktop' ? 'bg-btn-primary text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700'"
                        >
                        ...
                        <button
                          ...
                          class="p-1.5 rounded transition-colors"
                          :class="codeBlocks[parseInt(part)].previewMode === 'mobile' ? 'bg-btn-primary text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700'"
                        >
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        class="p-1.5 rounded transition-colors text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700"
                      >
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <Teleport to="body">
      <div
        ...
        class="fixed z-[9999] bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl py-1 min-w-[120px]"
        :style="{ left: contextMenuPos.x + 'px', top: contextMenuPos.y + 'px' }"
      >
        <button
          @click="copyAll"
          class="w-full px-4 py-2 text-left text-sm text-zinc-200 hover:bg-zinc-700 flex items-center gap-2"
        >
      ...
      >
        <div class="flex items-center justify-between px-4 py-3 bg-zinc-900 border-b border-zinc-800">
          <div class="flex items-center gap-2">
            <button
              ...
              :class="codeBlocks[fullscreenIndex].previewMode === 'desktop' ? 'bg-btn-primary text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700'"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
            ...
            <button
              ...
              class="p-2 rounded transition-colors"
              :class="codeBlocks[fullscreenIndex].previewMode === 'mobile' ? 'bg-btn-primary text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700'"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
          ...
          <button
            ...
            class="p-2 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700 transition-colors"
          >
        ...
        <div class="flex-1 flex items-center justify-center p-8">
          <div
            ...
            :style="codeBlocks[fullscreenIndex].previewMode === 'mobile' ? 'height: 90%; aspect-ratio: 9/20;' : 'width: 90%; aspect-ratio: 16/9;'"
          >
            <iframe
