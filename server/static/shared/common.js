...
async function fetchUIConfig() {
    ...
    const connected = await wsManager.waitForConnection();
    if (!connected) {
        console.error('WebSocket连接失败，无法获取UI配置');
        return { private_mode: false };
    }
    try {
        uiConfig = await wsManager.request('/api/ui-config', 'GET');
        return uiConfig;
    } catch (error) {
...
async function applyPrivateMode() {
    const config = await fetchUIConfig();
    if (config && config.private_mode) {
        const dialoguesLink = document.querySelector('a[href*="dialogues"]');
        if (dialoguesLink) {
            dialoguesLink.parentElement.style.display = 'none';
        }
    }
}
